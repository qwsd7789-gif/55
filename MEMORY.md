...
## 📅 时间线
### 2026-04-03
- 用户要求将即梦官方 CLI 提交流程固化成共享全局 skill，并同步给所有 agent 复用。
- 已新建共享技能：`seedance-cli`，安装位置：`C:\Users\Administrator\.openclaw\skills\seedance-cli`。
- 本机 Dreamina 官方 CLI 可用路径：`C:\Users\Administrator\dreamina-bin\dreamina.exe`。
- 后续当用户明确要求“提交即梦任务 / 查 submit_id / 查结果 / 查余额”时，默认优先走 `seedance-cli`，不是只写提示词。
- 默认视频模型偏好：`seedance2.0fast`，适合快速提交 15s fast 任务。
### 2026-03-16
