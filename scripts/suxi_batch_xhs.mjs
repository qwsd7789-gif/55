...
const prompt = '提取爆款元素 重绘制图片保持手串大小不变生成';
...
const wanted = [2, 3]; // 第3、4篇（0-based）
...
const runId = `suxi-batch-${Date.now()}`;
...
for (const idx of wanted) {
  ...
  const imgs = [...sec.matchAll(/<img src='([^']+)'/g)].map(m => m[1].replace(/\//g, path.sep)).slice(0,4);
  ...
  for (let i = 0; i < imgs.length; i++) {
    ...
    try {
      ...
      const ext = 'png';
      const outFile = path.join(outDir, `note${idx+1}-img${i+1}.${ext}`);
