...
const prompt = '提取爆款元素 重绘制图片 更换相近背景';
...
const wanted = [1, 2, 3]; // 第2/3/4篇（0-based）
...
const runId = `suxi-batch-2to4-top3-${Date.now()}`;
...
for (const idx of wanted) {
  ...
  const imgs = [...sec.matchAll(/<img src='([^']+)'/g)].map(m => m[1].replace(/\//g, path.sep)).slice(0,3);
