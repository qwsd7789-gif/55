...
const prompt = '参考这张图片的主题，重新生成一张新的图片。要求：保留核心元素，改变背景为室内暖光环境、极简风，改变角度，不要和原图一样。重要：不要输出任何解释文字、不要输出说明、不要输出markdown，只返回最终生成的图片结果。';
...
const wanted = [2,3,4]; // 第3/4/5篇 0-based
const imageIndexes = [1,2]; // 第2/3张 0-based
...
const runId = `suxi-batch-3to5-img23-warm-${Date.now()}`;
...
for (const idx of wanted) {
  ...
  const imgs = [...sec.matchAll(/<img src='([^']+)'/g)].map(m => m[1].replace(/\//g, path.sep));
  const note = { noteIndex: idx + 1, title, items: [] };
  console.log(`NOTE ${idx+1}: ${title}`);
  for (const imgIdx of imageIndexes) {
    const original = imgs[imgIdx];
    if (!original) {
      note.items.push({ imageNumber: imgIdx + 1, error: 'missing source image', ok: false });
      continue;
    }
    console.log(`  RUN ${imgIdx+1}: ${original}`);
    try {
      const outBuf = await callEdit(original);
      const outFile = path.join(outDir, `note${idx+1}-img${imgIdx+1}.png`);
      fs.writeFileSync(outFile, outBuf);
      note.items.push({ imageNumber: imgIdx + 1, original, generated: outFile, ok: true });
    } catch (e) {
      note.items.push({ imageNumber: imgIdx + 1, original, error: String(e.message || e), ok: false });
    }
  }
  manifest.push(note);
}

fs.writeFileSync(path.join(outDir, 'manifest.json'), JSON.stringify({ model, prompt, sourceHtml: htmlPath, manifest }, null, 2), 'utf8');
const htmlOut = path.join(outDir, 'result.html');
const rows = [];
rows.push(`<!doctype html><html><head><meta charset="utf-8"><title>Suxi Batch 3-5 Img2-3 Warm</title><style>body{font-family:Arial,Microsoft YaHei;padding:20px}section{border:1px solid #ddd;border-radius:12px;padding:12px;margin:12px 0}.grid{display:grid;grid-template-columns:repeat(2,minmax(320px,1fr));gap:16px;margin-top:12px}.card{border:1px solid #eee;border-radius:10px;padding:10px}.img{max-width:100%;max-height:420px;border:1px solid #ddd;border-radius:8px;background:#fafafa}.meta{font-size:13px;color:#555;word-break:break-all;margin:8px 0}</style></head><body>`);
rows.push(`<h1>Suxi 重绘结果（第3-5篇，第2/3张）</h1><p><b>模型：</b>${model}</p><p><b>提示词：</b>${prompt}</p><p><b>来源：</b>${htmlPath}</p><p><b>输出目录：</b>${outDir}</p>`);
for (const note of manifest) {
  rows.push(`<section><h2>第 ${note.noteIndex} 篇：${note.title}</h2><div class="grid">`);
  note.items.forEach((it) => {
    const orig = it.original ? it.original.replace(/\\/g,'/') : '';
    const gen = it.generated ? path.resolve(it.generated).replace(/\\/g,'/') : '';
    rows.push(`<div class="card"><h3>第 ${it.imageNumber} 张</h3><div class="meta"><b>原图：</b>${orig || '无'}</div>${orig ? `<img class="img" src="file:///${orig}" />` : ''}<div class="meta"><b>生成图：</b>${gen || '失败'}</div>${it.ok ? `<img class="img" src="file:///${gen}" />` : `<pre>${String(it.error||'').replace(/</g,'&lt;')}</pre>`}</div>`);
  });
