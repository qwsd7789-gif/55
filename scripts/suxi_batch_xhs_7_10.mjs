...
const prompt = '提取爆款元素 重绘制图片保持手串大小不变 包含 简约艺术自然元素风格';
...
const wanted = [6, 7, 8, 9]; // 第7/8/9/10篇（0-based）
...
const runId = `suxi-batch-7to10-${Date.now()}`;
...
for (const idx of wanted) {
  ...
  const imgs = [...sec.matchAll(/<img src='([^']+)'/g)].map(m => m[1].replace(/\//g, path.sep)).slice(0,2);
